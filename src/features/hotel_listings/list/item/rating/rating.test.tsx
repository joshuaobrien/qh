...
describe("Rating", () => {
	...
	it.each(kinds.flatMap((kind) => validCases.map((c) => ({ ...c, kind }))))(
		...
		({ rating, kind, full, half, empty }) => {
			...
			const expectedLabel = `Rating: ${rating} out of 5. (${
				kind === "star" ? "official rating" : "self rated"
			})`;
	...
	it.each(kinds.flatMap((kind) => edgeCases.map((c) => ({ ...c, kind }))))(
		...
		({ rating, kind, full, half, empty }) => {
			...
			const expectedLabel = `Rating: ${clampedRating} out of 5. (${
				kind === "star" ? "official rating" : "self rated"
			})`;
	...
	it.each(kinds.flatMap((kind) => ratings.map((rating) => ({ rating, kind }))))(
		"matches snapshot for rating $rating and kind $kind",
		({ rating, kind }) => {
