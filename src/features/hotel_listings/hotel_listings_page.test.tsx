...
import { FindHotelsResponse, HotelService } from "../../services/hotel_service/hotel_service";
  ...
  results: [
    {
      ...
      property: {
        ...
        previewImage: { url: "https://example.com/hotel_sydney.jpg", caption: "", imageType: "" },
    ...
    },
    {
      ...
      property: {
        ...
        previewImage: { url: "https://example.com/hotel_melbourne.jpg", caption: "", imageType: "" },
...
describe("HotelListingsPage", () => {
  ...
  it("renders error state", async () => {
    vi.mocked(hotelService).findHotels.mockRejectedValue(new Error("Failed to fetch hotels"));
    renderWithQueryClient(<HotelListingsPage hotelService={hotelService} />);
    await waitFor(() => {
      expect(screen.queryByRole("list")).toBeNull();
      expect(screen.getByText("Sorry, there was a problem finding hotels.")).toBeDefined();
    });
  });
  ...
  it("applies default sorting order on initial render", async () => {
    renderWithQueryClient(<HotelListingsPage hotelService={hotelService} />);
    expect(vi.mocked(hotelService).findHotels).toHaveBeenLastCalledWith({ sortOrder: "high-first" });
  });
  ...
  it("changes sort order and verifies correct sorting", async () => {
    ...
    expect(vi.mocked(hotelService).findHotels).toHaveBeenLastCalledWith({ sortOrder: "low-first" });
  ...
  it("only advertises free cancellation if there is a free cancellation offer", async () => {
    ...
    expect(within(hotelSydneyListItem!).queryByText("Free cancellation")).toBeDefined();
    expect(screen.getAllByText("Free cancellation")).toHaveLength(1);
  })

  it("matches snapshot for a loaded list of hotels", async () => {
    vi.mocked(hotelService).findHotels.mockResolvedValue(mockHotelsData);
    const { container } = renderWithQueryClient(<HotelListingsPage hotelService={hotelService} />);
